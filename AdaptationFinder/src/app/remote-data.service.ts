...
export class RemoteDataService {
  ...
  fetchBook(isbn:string): any {
    ...
    .then(result => ({"title":result.title, "url":result.url, "isbn":isbn, "author_name":result.authors.reduce((authors, author) => ([...authors, author.name]), []), "cover":"http://covers.openlibrary.org/b/isbn/" + isbn + "-M.jpg"}))
    .catch(error => null);
  }
